...
#include "misc.h"
    ...
    static inline void delayMicroseconds(uint32_t _delayTime)
    {
        SYSTICK_DelayUs(_delayTime);
    }
    ...
    extern uint8_t currentSsbStatus;
    extern int8_t audioMuteMcuPin;


    //==================== FUNCITON LIST  === REDFINE BY IANLEE
...
//public: =============== IANLEE CHECK
    ...
    //void SI4735_setup1(uint8_t resetPin, uint8_t ctsIntEnable, uint8_t defaultFunction, uint8_t audioMode = SI473X_ANALOG_AUDIO, uint8_t clockType = XOSCEN_CRYSTAL, uint8_t gpo2Enable = 0);
    void SI4735_setup1(uint8_t resetPin, uint8_t ctsIntEnable, uint8_t defaultFunction, uint8_t audioMode, uint8_t clockType, uint8_t gpo2Enable);

    void setRefClock(uint16_t refclk);
    void setRefClockPrescaler(uint16_t prescale, uint8_t rclk_sel); // rclk_sel = 0
     ...
     */
    static inline void setAvcAmMaxGain_Default()
    {
     ...
     */
    static inline void setAmSoftMuteMaxAttenuation(uint8_t smattn) // = 0
    {
     ...
     * @param smattn Maximum attenuation to apply when in soft mute.
     */
    static inline void setSsbSoftMuteMaxAttenuation(uint8_t smattn)  //smattn = 0
    {
     ...
     */
    static inline void setSsbIfAgcReleaseRate(uint8_t param) // param = 140
    {
     ...
     */
    static inline void setSsbIfAgcAttackRate(uint8_t param)  //uint8_t param = 4
    {
    ...
    void setSsbAgcOverrite(uint8_t SSBAGCDIS, uint8_t SSBAGCNDX, uint8_t reserved); //uint8_t reserved = 0
     ...
     */
    static inline uint8_t  SI4735_getCurrentRSSI()
    {
     ...
     */
    static inline uint8_t  SI4735_getCurrentSNR()
    {
     ...
     */
    static inline bool  SI4735_getCurrentPilot()
    {
     ...
     * @details Same SI4735_volumeUp()
     * @see volumeUp
     */
    static inline void setVolumeUp() { SI4735_volumeUp(); };
    /**
     * @ingroup group13 Audio volume
     * @brief Set the Volume Down
     * @details Same SI4735_volumeDown()
     * @return voi
     */
    static inline void setVolumeDown() { SI4735_volumeDown(); };

    /**
     * @ingroup group13 Digital Audio setup
     * @brief Sets the Audio Mode. See table below.
     * @details If you want to change the audio mode, call this function before call SI4735_setAM(), SI4735_setFM() or setSSB().
     * @details Sets the Si47XX device to use ANALOG or DIGITAL audio output. The table below shows the valid values.
     * @details This function will only take effect after calling SI4735_setAM(), SI4735_setFM() or setSSB().
     *
     *
     ...
     * @param smattn Maximum attenuation to apply when in soft mute
     */
    static inline void setFmSoftMuteMaxAttenuation(uint8_t smattn) //uint8_t smattn = 0
    {
     ...
     */
    static inline void setFmNoiseBlank(uint16_t nb_rate, uint16_t nb_interval, uint16_t nb_irr_filter)
    {
    ...
    static inline void setFmNoiseBlankDefault()
    {
        uint16_t nb_rate = 64; uint16_t nb_interval = 55; uint16_t nb_irr_filter = 300;
        sendProperty(FM_NB_RATE, nb_rate);
        sendProperty(FM_NB_INTERVAL, nb_interval);
        sendProperty(FM_NB_IIR_FILTER, nb_irr_filter);
    }
    ...
    static inline void setAmNoiseBlank(uint16_t nb_rate, uint16_t nb_interval, uint16_t nb_irr_filter)
    ...
    static inline void setAmNoiseBlank_Default()
    {
        uint16_t nb_rate = 64; uint16_t nb_interval = 55; uint16_t nb_irr_filter = 300;
        sendProperty(AM_NB_RATE, nb_rate);
        sendProperty(AM_NB_INTERVAL, nb_interval);
     ...
     */
    static inline bool SI4735_isCurrentTuneFM()
    {
     ...
     * @param filter_value
     */
    static inline void setFmBandwidth(uint8_t filter_value)  //filter_value = 0
    {
     ...
     * @see frequencyUp
     */
    static inline void setFrequencyUp() { SI4735_frequencyUp(); };
     ...
     * @see frequencyDown
     */
    static inline void setFrequencyDown() { SI4735_frequencyDown(); };
    ...
    static inline void  SI4735_seekStationUp()
    {
        seekStationProgress2(NULL, SEEK_UP);
    };
    ...
    static inline void  SI4735_seekStationDown()
    {
        seekStationProgress2(NULL, SEEK_DOWN);
    };
     ...
     * @param value
     */
     static inline void setSeekAmSrnThreshold(uint16_t value) { sendProperty(AM_SEEK_SNR_THRESHOLD, value); }; // Wrong name! Will be removed later
     ...
     */
     ...
     static inline void setSeekAmSNRThreshold(uint16_t value) { sendProperty(AM_SEEK_SNR_THRESHOLD, value); }; // Fixing the function name
     ...
     * @param value
     */
    static inline void  setSeekFmSrnThreshold(uint16_t value) { sendProperty(FM_SEEK_TUNE_SNR_THRESHOLD, value); }; // Wrong name. Will be removed later
     ...
     */
    static inline void  setSeekFmSNRThreshold(uint16_t value) { sendProperty(FM_SEEK_TUNE_SNR_THRESHOLD, value); }; // Fixing the function name
    ...
    //  * @see setRdsConfig()
    //  */
    // static inline void clearRdsBuffer() { RdsInit();};
    ...
    //  * @see getRdsStatus
    //  */
    // static inline void rdsBeginQuery() { getRdsStatus(0, 0, 0); };
    ...
    //  */
    // static inline void getRdsStatus_Default()
    // {
    //     getRdsStatus(0, 0, 0);
    // }
     ...
     * @param SBCUTFLT
     */
    static inline void  SI4735_setSBBSidebandCutoffFilter(uint8_t SBCUTFLT) { setSSBSidebandCutoffFilter(SBCUTFLT); }; // Wrong name! will be removed later.
    ...
    void loadPatch(const uint8_t *ssb_patch_content, const uint16_t ssb_patch_content_size, uint8_t ssb_audiobw); //uint8_t ssb_audiobw = 1
    void SI4735_loadCompressedPatch(const uint8_t *ssb_patch_content, const uint16_t ssb_patch_content_size, const uint16_t *cmd_0x15, const int16_t cmd_0x15_size, uint8_t ssb_audiobw);  //uint8_t ssb_audiobw = 1
     ...
     * @details Example:
     * @code
     * SI4735_setFM(6400,10800,10390,10);
     *  SI4735_setFrequencyStep(100); // the step will be 1MHz (you are using FM mode)
     * .
     * .
     * .
     * SI4735_setAM(7000,7600,7100,5);
     *  SI4735_setFrequencyStep(1); // the step will be 1kHz (you are usin AM or SSB mode)
     * @endcode
     *
     * @see SI4735_setFM()
     * @see SI4735_setAM()
     ...
     */
    static inline void SI4735_getStatus_Default()
    {
...
//    void setDeviceOtherI2CAddress(uint8_t i2cAddr);
     ...
     */
    // static inline void setI2CStandardMode(void) { Wire_SetClock(100000); };
     ...
     */
    // static inline void setI2CFastModeCustom(long value) { Wire_SetClock(value); };  //long value = 500000
    ...
    void convertToChar(uint16_t value, char *strValue, uint8_t len, uint8_t dot, uint8_t separator, bool remove_leading_zeros); //bool remove_leading_zeros = true);
