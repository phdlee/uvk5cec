...
*/
//=================================================================
//IANLEE ADDED SOFT I2C
//-----------------------------------------------------------------

/*
  void BackLightBlink(int bCount)
  {
...
*/
/*
    void delay(uint32_t _delayTime)
    {
        SYSTICK_DelayUs(_delayTime * 1000);
    }
*/
    void Wire_Begin()
    {
    ...
    #define _txBufferSize 32
    uint8_t _txBufferIndex; // Index into buffer

    int Wire_SetClock(uint32_t _clockSpeed)
    {
      //return Wire.setClock(_clockSpeed);
      return 0;
    }
    ...
    int Wire_Write(uint8_t writeByte)
    {
        if (_txBufferIndex >= _txBufferSize) {
            return 0;
        }

        _txBuffer[_txBufferIndex++] = writeByte;
        return 1;
    }
    
    int Wire_WriteArray(uint8_t *writeBuff, uint8_t _writeSize)
    {
       uint8_t r = 0;
        for (uint8_t i = 0; i < _writeSize; ++i) {
            r += Wire_Write(writeBuff[i]);
        }
        return r;
    }

    void Wire_BeginTransmission(uint8_t _deviceAddress)
    {
        _txAddress = _deviceAddress;
        _txBufferIndex = 0;
    }
    ...
    int Wire_EndTransmission()
    {
	    ...
	    I2C_Start();
        //	I2C_Write(0xA0);
	    I2C_Write((_txAddress << 1) | I2C_WRITE);
	    //I2C_Write((Register << 1) | I2C_WRITE);
	    //Value = ((Value >> 8) & 0xFF) | ((Value & 0xFF) << 8);
	    //_WriteCount = I2C_WriteBuffer(&_txBuffer, _txBufferIndex);

        for (int i = 0; i < _txBufferIndex; i++)
        {
            r = I2C_Write(_txBuffer[i]);
            if (r < 0)
                return -1;
        }
    ...
    uint8_t Wire_RequestFrom(uint8_t _deviceAddress, uint8_t _response_size)
    {
        //Wire.requestFrom(_deviceAddress, _response_size);
        /*
        I2C_Start();
        I2C_Write(0xA0);
        ...
        */
	    ...
	    I2C_Start();
	    //I2C_Write(0x80);
	    I2C_Write((_deviceAddress << 1) | I2C_READ);
	    _rxBufferBytesRead = I2C_ReadBuffer(&_rxBuffer, _response_size);
	    I2C_Stop();  
...
//IANLEE VAR REDEFINE
//----------------------------------------------------------------
  //DEFINE VARIABLE ====================== IANLEE REDEFINE

    //Not use RDS for reduce program memory
    ...
    #define deviceAddress SI473X_ADDR_SEN_LOW
    ...
    #define maxDelaySetFrequency  MAX_DELAY_AFTER_SET_FREQUENCY
    #define maxDelayAfterPouwerUp  MAX_DELAY_AFTER_POWERUP
    ...
    #define maxSeekTime MAX_SEEK_TIME
...
//ianlee 이성호
    ...
    #define currentAudioMode  SI473X_ANALOG_AUDIO
...
si47x_status getInterruptStatus()
{
    ...
    Wire_Write(GET_INT_STATUS);
    Wire_EndTransmission();

    Wire_RequestFrom(deviceAddress, 1);
    status.raw = Wire_Read();
...
//  */
// int16_t getDeviceI2CAddress(uint8_t _resetPin)
// {
//     int16_t error;
...
//     Wire_BeginTransmission(SI473X_ADDR_SEN_LOW);
...
//     Wire_BeginTransmission(SI473X_ADDR_SEN_HIGH);
//     error = Wire_EndTransmission();
//     if (error == 0)
//     {
...
void radioPowerUp(void)
{
    ...
    Wire_BeginTransmission(deviceAddress);
    Wire_Write(POWER_UP);
    Wire_Write(powerUp.raw[0]); // Content of ARG1
    Wire_Write(powerUp.raw[1]); // COntent of ARG2
    Wire_EndTransmission();
...
void getFirmware(void)
{
    ...
    do
    {
        ...
        Wire_RequestFrom(deviceAddress, 9);
        for (int i = 0; i < 9; i++)
            firmwareInfo.raw[i] = Wire_Read();
 ...
 */
void SI4735_setup1(uint8_t _resetPin, uint8_t _ctsIntEnable, uint8_t _defaultFunction, uint8_t _audioMode, uint8_t _clockType, uint8_t _gpo2Enable)
{
    Wire_Begin();

    resetPin = _resetPin;
    ctsIntEnable = (_ctsIntEnable != 0) ? 1 : 0; // Keeps old versions of the sketches running
    gpo2Enable = _gpo2Enable;
    //currentAudioMode = _audioMode;
    ...
    setPowerUp(_ctsIntEnable, _gpo2Enable, 0, _clockType, _defaultFunction, _audioMode);
 ...
 */
void SI4735_setup2(uint8_t resetPin, uint8_t defaultFunction)
{
...
void SI4735_setFrequency(uint16_t freq)
{
    ...
    Wire_BeginTransmission(deviceAddress);
    Wire_Write(currentTune);
    Wire_Write(currentFrequencyParams.raw[0]); // Send a byte with FAST and  FREEZE information; if not FM must be 0;
    Wire_Write(currentFrequencyParams.arg.FREQH);
    Wire_Write(currentFrequencyParams.arg.FREQL);
    Wire_Write(currentFrequencyParams.arg.ANTCAPH);
    // If current tune is not FM sent one more byte
    if (currentTune == AM_TUNE_FREQ) // if AM or SSB
        Wire_Write(currentFrequencyParams.arg.ANTCAPL);
...
void SI4735_setBandwidth(uint8_t AMCHFLT, uint8_t AMPLFLT)
{
    ...
    //volume = volume;
    Wire_BeginTransmission(deviceAddress);
    Wire_Write(SET_PROPERTY);
    Wire_Write(0x00);                  // Always 0x00
    Wire_Write(property.raw.byteHigh); // High byte first
    Wire_Write(property.raw.byteLow);  // Low byte after
    Wire_Write(filter.raw[1]);         // Raw data for AMCHFLT and
    Wire_Write(filter.raw[0]);         // AMPLFLT
    Wire_EndTransmission();
    waitToSend();
}
...
void getAutomaticGainControl()
{
    ...
    {
        waitToSend();
        Wire_RequestFrom(deviceAddress, 3);
        currentAgcStatus.raw[0] = Wire_Read(); // STATUS response
        currentAgcStatus.raw[1] = Wire_Read(); // RESP 1
        currentAgcStatus.raw[2] = Wire_Read(); // RESP 2
...
void SI4735_setAutomaticGainControl(uint8_t AGCDIS, uint8_t AGCIDX)
{
    ...
    Wire_BeginTransmission(deviceAddress);
    Wire_Write(cmd);
    Wire_Write(agc.raw[0]);
    Wire_Write(agc.raw[1]);
    Wire_EndTransmission();
 ...
 *
 * @details This method sould be called berore call  SI4735_getCurrentRSSI(),  SI4735_getCurrentSNR() etc.
...
void SI4735_getCurrentReceivedSignalQuality(uint8_t INTACK)
{
    ...
    //{
    waitToSend();
    Wire_RequestFrom(deviceAddress, sizeResponse);
    // Gets response information
    for (uint8_t i = 0; i < sizeResponse; i++)
        currentRqsStatus.raw[i] = Wire_Read();
 ...
 *        1 = Clears RSQINT, BLENDINT, SNRHINT, SNRLINT, RSSIHINT, RSSILINT, MULTHINT, MULTLINT.
 */
//oid SI4735_getCurrentReceivedSignalQualityvoid)
//{
...
void seekStation(uint8_t SEEKUP, uint8_t WRAP)
{
    ...
    Wire_BeginTransmission(deviceAddress);
    Wire_Write(seek_start_cmd);
    Wire_Write(seek.raw); // ARG1
    ...
    if (seek_start_cmd == AM_SEEK_START) // Sets additional configuration for AM mode
    {
        ...
        Wire_Write(seek_am_complement.ARG2);                                // ARG2 - Always 0
        Wire_Write(seek_am_complement.ARG3);                                // ARG3 - Always 0
        Wire_Write(seek_am_complement.ANTCAPH);                             // ARG4 - Tuning Capacitor: The tuning capacitor value
        Wire_Write(seek_am_complement.ANTCAPL);                             // ARG5 - will be selected automatically.
    }
 ...
 */
void seekStationProgress2(void (*showFunc)(uint16_t f), uint8_t up_down)
{
    /*
    si47x_frequency freq;
    ...
    */
    ...
    do
    {
        ...
        if (showFunc != NULL)
            showFunc(freq.value);
    } while (!currentStatus.resp.VALID && !currentStatus.resp.BLTF && (elapsed_seek++) < maxSeekTime);
}
...
void sendProperty(uint16_t propertyNumber, uint16_t parameter)
{
    ...
    Wire_Write(SET_PROPERTY);
    Wire_Write(0x00);
    Wire_Write(property.raw.byteHigh); // Send property - High byte - most significant first
    Wire_Write(property.raw.byteLow);  // Send property - Low byte - less significant after
    Wire_Write(param.raw.byteHigh);    // Send the argments. High Byte - Most significant first
    Wire_Write(param.raw.byteLow);     // Send the argments. Low Byte - Less significant after
    Wire_EndTransmission();
...
void getCommandResponse(int response_size, uint8_t *response)
{
    ...
    Wire_RequestFrom(deviceAddress, response_size);
    // Gets response information
    for (byte i = 0; i < response_size; i++)
        response[i] = Wire_Read();
}
 ...
 */
int32_t getProperty(uint16_t propertyNumber)
{
    si47x_property property;
    si47x_status status;
    ...
    waitToSend();
    Wire_RequestFrom(deviceAddress, 4);
    status.raw = Wire_Read();
    ...
    property.raw.byteHigh = Wire_Read();
    property.raw.byteLow = Wire_Read();
 ...
 */
void SI4735_setVolume(uint8_t _volume)
{
    sendProperty(RX_VOLUME, _volume);
    //volume = _volume;
}
...
//     Wire_BeginTransmission(deviceAddress);
//     Wire_Write(SET_PROPERTY);
//     Wire_Write(0x00);                  // Always 0x00 (I need to check it)
//     Wire_Write(property.raw.byteHigh); // Send property - High byte - most significant first
//     Wire_Write(property.raw.byteLow);  // Low byte
//     Wire_Write(config.raw[1]);         // Send the argments. Most significant first
//     Wire_Write(config.raw[0]);
//     Wire_EndTransmission();
//     delayMicroseconds(550);
...
//     Wire_Write(property.raw.byteHigh); // Send property - High byte - most significant first
//     Wire_Write(property.raw.byteLow);  // Low byte
//     Wire_Write(rds_int_source.raw[1]); // Send the argments. Most significant first
//     Wire_Write(rds_int_source.raw[0]);
//     Wire_EndTransmission();
//     waitToSend();
// }
...
//     Wire_BeginTransmission(deviceAddress);
//     Wire_Write(FM_RDS_STATUS);
//     Wire_Write(rds_cmd.raw);
//     Wire_EndTransmission();
...
//         waitToSend();
//         // Gets response information
//         Wire_RequestFrom(deviceAddress, 13);
//         for (uint8_t i = 0; i < 13; i++)
//             currentRdsStatus.raw[i] = Wire_Read();
...
// {
//     rdsBeginQuery();
//     if (!getRdsReceived())  return false;
//     if (!getRdsSync() || getNumRdsFifoUsed() == 0) return false;
 ...
 */
void SI4735_setSSBBfo2(int offset)
{
    ...
    Wire_Write(property.raw.byteHigh); // High byte first
    Wire_Write(property.raw.byteLow);  // Low byte after
    Wire_Write(bfo_offset.raw.FREQH);  // Offset freq. high byte first
    Wire_Write(bfo_offset.raw.FREQL);  // Offset freq. low byte first
 ...
 */
void SI4735_setSSB2(uint8_t usblsb)
{
...
void sendSSBModeProperty()
{
    ...
    Wire_Write(property.raw.byteHigh); // High byte first
    Wire_Write(property.raw.byteLow);  // Low byte after
    Wire_Write(currentSSBMode.raw[1]); // SSB MODE params; freq. high byte first
    Wire_Write(currentSSBMode.raw[0]); // SSB MODE params; freq. low byte after
...
si47x_firmware_query_library queryLibraryId()
{
    ...
    Wire_Write(POWER_UP);
    //ianlee 2024.01.30
    Wire_Write(0b00011111);          // Set to Read Library ID, disable interrupt; disable GPO2OEN; boot normaly; enable External Crystal Oscillator  .
    //Wire_Write(0b00001111);          // Set to Read Library ID, disable interrupt; disable GPO2OEN; boot normaly; enable External Crystal Oscillator  .
    Wire_Write(SI473X_ANALOG_AUDIO); // Set to Analog Line Input.
    Wire_EndTransmission();

    do
    {
        waitToSend();
        Wire_RequestFrom(deviceAddress, 8);
        for (int i = 0; i < 8; i++)
            libraryID.raw[i] = Wire_Read();
...
void patchPowerUp()
{
    ...
    Wire_Write(POWER_UP);
    //ianlee 2024.01.30
    Wire_Write(0b00110001);          // This is a condition for loading the patch: Set to AM, Enable External Crystal Oscillator; Set patch enable; GPO2 output disabled; CTS interrupt disabled. You can change this calling setSSB.
    //Wire_Write(0b00100001);          // This is a condition for loading the patch: Set to AM, Enable External Crystal Oscillator; Set patch enable; GPO2 output disabled; CTS interrupt disabled. You can change this calling setSSB.
    Wire_Write(SI473X_ANALOG_AUDIO); // This is a condition for loading the patch: Set to Analog Output. You can change this calling setSSB.
    Wire_EndTransmission();
    delay(maxDelayAfterPouwerUp);
}
...
void ssbPowerUp()
{
    ...
    Wire_Write(POWER_UP);
    //ianlee 2024.01.30     
    //Wire_Write(0b00010001); // This is a condition for loading the patch: Set to AM, Enable External Crystal Oscillator; Set patch enable; GPO2 output disabled; CTS interrupt disabled. You can change this calling setSSB.
    Wire_Write(0b00000001); // This is a condition for loading the patch: Set to AM, Enable External Crystal Oscillator; Set patch enable; GPO2 output disabled; CTS interrupt disabled. You can change this calling setSSB.
    Wire_Write(0b00000101); // This is a condition for loading the patch: Set to Analog Output. You can change this calling setSSB.
    Wire_EndTransmission();
    delayMicroseconds(2500);
...
si4735_eeprom_patch_header downloadPatchFromEeprom(int eeprom_i2c_address)
{
    ...
    Wire_BeginTransmission(eeprom_i2c_address);
    Wire_Write(0x00); // offset Most significant Byte
    Wire_Write(0x00); // offset Less significant Byte
    Wire_EndTransmission();
    delay(5);
    ...
    {
        Wire_RequestFrom(eeprom_i2c_address, 8);
        for (int i = k; i < (k + 8); i++)
            eep.raw[i] = Wire_Read();
    }
    ...
    for (i = 0; i < (int)eep.refined.patch_size; i += 8)
    {
        ...
        Wire_BeginTransmission(eeprom_i2c_address);
        Wire_Write((int)offset >> 8);   // header_size >> 8 wil be always 0 in this case
        Wire_Write((int)offset & 0XFF); // offset Less significant Byte
        Wire_EndTransmission();

        Wire_RequestFrom(eeprom_i2c_address, 8);
        for (int j = 0; j < 8; j++)
        {
            bufferAux[j] = Wire_Read();
        }

        Wire_BeginTransmission(deviceAddress);
        Wire_WriteArray(bufferAux, 8);
        Wire_EndTransmission();
...
void patchPowerUpNBFM()
{
    ...
    Wire_BeginTransmission(deviceAddress);
    Wire_Write(POWER_UP);
    Wire_Write(0b00110000);          // This is a condition for loading the patch: Set to AM, Enable External Crystal Oscillator; Set patch enable; GPO2 output disabled; CTS interrupt disabled.
    Wire_Write(SI473X_ANALOG_AUDIO); // This is a condition for loading the patch: Set to Analog Output. You can change this calling setNBFM.
    Wire_EndTransmission();
    delay(maxDelayAfterPouwerUp);
}
 ...
 * @see SI4735_setAM(), setSSB(), SI4735_setFM()
 * @see  SI4735_setFrequencyStep()
 * @see void setFrequency(uint16_t freq)
 */
void setNBFM2()
 ...
 * @see SI4735_setAM(), SI4735_setFM(), setSSB()
 * @see  SI4735_setFrequencyStep()
 * @see void setFrequency(uint16_t freq)
 *
 * @param fromFreq minimum frequency for the band
...
void setFrequencyNBFM(uint16_t freq)
{
    ...
    Wire_BeginTransmission(deviceAddress);
    Wire_Write(0x50);
    Wire_Write(0x00); // Send a byte with FAST and  FREEZE information; if not FM must be 0;
    Wire_Write(currentFrequency.raw.FREQH);
    Wire_Write(currentFrequency.raw.FREQL);
    Wire_EndTransmission();
    waitToSend();                // Wait for the si473x is ready.
    currentWorkFrequency = freq; // check it
