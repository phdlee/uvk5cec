...
void UI_PrintStringSmallBufferNormal35(const char *pString, uint8_t * buffer)
{
	//UI_PrintStringBuffer(pString, buffer, ARRAY_SIZE(gFont3x5[0]), (uint8_t *)gFont3x5);
	uint8_t * font = (uint8_t *)gFont3x5;
	uint8_t char_width = ARRAY_SIZE(gFont3x5[0]);
	const size_t Length = strlen(pString);
	const unsigned int char_spacing = char_width + 1;
	for (size_t i = 0; i < Length; i++) {
		const unsigned int index = pString[i] - ' ' - 1;
