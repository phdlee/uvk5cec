...
#define IS_RIGINFO(x)     	   ((x) >= RIGINFO_FIRST && (x) <= RIGINFO_LAST)
#define IS_CWMSG(x)     	   ((x) >= CWMSG_FIRST && (x) <= CWMSG_LAST)
#define IS_VALID_CHANNEL(x)    ((x) < LAST_CHANNEL)
#define IS_NOAA_CHANNEL(x)     ((x) >= NOAA_CHANNEL_FIRST && (x) <= NOAA_CHANNEL_LAST)

enum {
	MR_CHANNEL_FIRST   = 0,
	MR_CHANNEL_LAST    = 169u,

	//==========================
	//KD8CEC 
	RIGINFO_FIRST 	   = 170u,
	RIGINFO_MENU_LAST  = 180u,	//Display Menu
	RIGINFO_CWFIRST    = 181u,	//Display Menu
	RIGINFO_CWLAST     = 190u,	//Display Menu
	RIGINFO_LAST  	   = 190u,	//Last of RIG Info
	//RIGINFO_LAST  	   = 199u,	RESERVE(11) STRING VAR : MAX 20
	//END OF KD8CEC CODE==========
...
//Move to ceccommon.h for reduce use ram rate
