...
uint8_t CW_Tone       = 70;     //[EEPROM] *10 Hz, because BK4819 inc/dec 10Hz, 
uint8_t CW_WPM        = 17;     //[EEPROM] 5~ 50 
uint8_t CW_TXDelay    = 20;     //[EEPROM] * 100 milisecond


#ifdef ENABLE_CEC_CWTX_EXPERT
  uint8_t CW_KeyType    = CW_KEYTYPE_PADDLEA;  //[EEPROM]
#else
  uint8_t CW_KeyType    = CW_KEYTYPE_PADDLE;  //[EEPROM]
#endif

uint8_t CW_SPEED      = 10;     // MILISECOND
uint8_t CW_SideTone   = 1;    //[EEPROM but Not yet decided, will not use it, will use it as a volume]1: Enabled,  0: Disabled, not use some memory for this code
...
unsigned char update_PaddleLatch(uint8_t isUpdateKeyState)
{
  unsigned char tmpKeyerControl = 0;
  
  uint16_t paddle = GetCWADC();
  //diagnostic, VU2ESE
  //itoa(paddle, b, 10);
  //printLine2(b);
...
*/  
  ...
  //REVERSE
  if (paddle > CW_ADC.CWKKEY_DIT_AdcFrom  && paddle < CW_ADC.CWKKEY_DAH_AdcFrom)
    tmpKeyerControl |= DIT_L;
  else if (paddle > CW_ADC.CWKKEY_DAH_AdcFrom && paddle < CW_ADC.CWKKEY_BOTH_AdcFrom)
    tmpKeyerControl |= DAH_L;
  else if (paddle > CW_ADC.CWKKEY_BOTH_AdcFrom && paddle < CW_ADC.CWKKEY_BOTH_AdcTo)
...
void CWTXStart(uint8_t isDecoding, uint8_t justStart)
{
  ...
  */
  ...
  else if (CW_Mode == CWMODE_CWFM || CW_Mode == CWMODE_CW)
  {
 	  RADIO_PrepareTX();
  ...
  {
 	  RADIO_PrepareTX();
    if (VfoState[0] != VFO_STATE_NORMAL)  //Check TX Enable
      return;
	  BK4819_EnterTxMute();
  ...
  */
...
#ifdef ENABLE_CEC_CWTX_EXPERT
  //timer start
  CECTimer0Enable(CEC_TIMER_MSEC);
#endif
  ...
  int _chkTimeOut = 0;
#ifdef ENABLE_CEC_CWTX_EXPERT
  while ( update_PaddleLatch(0) != IDLE &&  ++_chkTimeOut < 1000)
    SYSTEM_DelayMs(1);
  ...
  uint32_t lastKeyPressTime = 0;
...
#ifdef ENABLE_CEC_CWTX_EXPERT
    //EXPERT MODE WITH TIMER AMD IAMBICA,B
    //if (CW_KeyType == CW_KEYTYPE_STRAIGHT || CW_KeyType == CW_KEYTYPE_KEYPAD_ST )
    if (CW_KeyType == CW_KEYTYPE_STRAIGHT )
    {
      tmpKeyControl = update_PaddleLatch(0);      
      if (tmpKeyControl == DAH_L || tmpKeyControl == DIT_L || tmpKeyControl == (DAH_L | DIT_L))
      {
        CWKeyDown();
        lastPressKeyTime = millis10();
      }
    ...
    else  // if (CW_KeyType == CW_KEYTYPE_PADDLEA || CW_KeyType == CW_KEYTYPE_PADDLEB)
    {
      ...
      while(continue_loop) 
      {
        switch (keyerState) {
          case IDLE:
            tmpKeyControl = update_PaddleLatch(0);
            if ( tmpKeyControl == DAH_L || tmpKeyControl == DIT_L || tmpKeyControl == (DAH_L | DIT_L) || (keyerControl & 0x03)) 
            {
          ...
          case CHK_DIT:
            ...
            if (isDecoding == 1 && pressKeyIndex < 9)      
          ...
          case KEYED:
            ...
            else if (CW_KeyType == CW_KEYTYPE_PADDLEB)
            {
      ...
      } //end of loop for iambic

     //======================================== end of test ron's code
#else
    //WITH KEYPAD PADDLE 
